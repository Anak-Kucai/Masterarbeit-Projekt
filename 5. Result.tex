...
Table~\ref{tab:yield_summary} summarizes the data obtained from the pyrolysis and biochar yield calculated using Equation~\ref{eq:biochar yield}
...
\begin{tabular}{llccc}
\toprule
\multirow{2}{*}{\textbf{Sample}} & \multirow{2}{*}{\textbf{$T_\mathrm{pyr}$ (\textdegree C)}} & \multicolumn{2}{c}{\textbf{Mass (g)}} & \multirow{2}{*}{\textbf{Yield (\%)}} \\
\cmidrule(lr){3-4}
...
\footnotesize
\textit{Note.} ``$T_\mathrm{pyr}$'' refers to the pyrolysis temperature at which the biochar samples were produced. 
\end{minipage}
\end{table}

Figure~\ref{fig:pyrolysis-yield} also provides a visual overview of how the biochar yield changed across the investigated temperature range for both digestates.
...
As shown in Figure~\ref{fig:pyrolysis-yield}, the biochar yields from both digestates behave identically,  with the replicate standard deviation staying below half a percentage point throughout. Across the temperature range, the French digestate exhibited higher solid yield, falling from 45.7\,\% at 400\,$^{\circ}$C to 42.2\,\% at 600\,$^{\circ}$C, whereas the Algerian digestate yielded between 37.4\,\% and 33.5\,\% over the same interval, as shown in both Table~\ref{tab:yield_summary} and Figure~\ref{fig:pyrolysis-yield}. The French biochar therefore exceeded the Algerian biochar by roughly eight to nine percentage points at every temperature. This difference may be attributed to  different feedstock composition between both digestates rather than the pyrolysis process because both digestates were derived from slaughterhouse waste and pyrolyzed under identical conditions. The higher French biochar yield indicates a larger fraction of material retained in the solid phase, which points to differences in the organic and mineral balance of the two digestates arising from their respective origins and upstream digestion. This interpretation was addressed further in Section~\ref{sec:characterisation} based on the elemental and ash data, where the mineral content recovered from the coal is directly quantified.

\vspace{0.3em}

Figure~\ref{fig:pyrolysis-yield} also shows clearly that both biochar yield decreased steadily as the pyrolysis temperature increased, amounting to about 3.6 percentage points for the French and 4.0 for the Algerian biochar between 400 and 600\,$^{\circ}$C. The downward trend follows the expected release of volatile matter at higher temperatures since the thermally less stable fractions are removed and a more carbonized residue remains (Section~\ref{subsubsec:Effect of Temperature on Biochar}). The modest size of the decline is itself informative: in digestate-derived feedstocks, the high mineral
...
As mentioned in section \ref{Physical Activation of Biochar}, steam gasification is the pore-forming reaction, which depends heavily on a larger steam supply and temperatures above 700\,$^{\circ}$C used here. The 0.5\,mL\,g$^{-1}$ charge may or may not reach a greater carbon conversion despite its smaller total mass loss. Mass loss alone cannot fully differentiate the two contributions. Therefore, this aspect is further evaluated using the BET results. Therefore, the 0.5\,mL\,g$^{-1}$ dose was retained as the standard for its more reliable wetting of the char and lower mass loss.

\begin{table}[ht]
\centering
\caption{Activation mass loss from CO$_2$ activation for the three digestate-derived biochars of
the previous project work
...
\begin{axis}[
    width=14cm,
    height=9cm,
    title={Activation mass loss of Algerian activated biochar}
...
The CO$_2$-activation of the previous project work offers a direct point of comparison since it also used the same reactor, temperature, and residence time as the steam-activation runs here (700\,$^{\circ}$C, 1\,h). These two slaughterhouse-digestates, though they come from different sources, could provide insight into this discussion. Since no water is sprayed during CO$_2$ activation, the reported before/after loss is assumed to be under low-moisture conditions and can be placed alongside the corrected steam values without adjustment. 

\vspace{0.3em}

Based on this, the steam-activated char loses more mass than the CO$_2$-activated char at every pyrolysis temperature (Figure~\ref{fig:co2-vs-steam}; Table~\ref{tab:co2-activation-projektarbeit}). The widest gap could be seen at 600\,$^{\circ}$C, where steam removes about 10--11\,\% compared to roughly 2\,\% for CO$_2$. 
...
The mass gain of each sample after the sulfur loading process was calculated from the difference
between the mass before and after the sulfur loading. The adsorption capacity of each sample was then calculated using Equation~\ref{Equation sulfur loading} and summarized together with the collected data in Table~\ref{tab:sulfur_loading}. 

\begin{table}[H]
\centering
\caption{adsorption capacity of the steam-activated French biochars.}
...
Table~\ref{tab:sulfur_loading} shows that all biochars exhibited an increase in mass after the sulfur loading process. This indicates that elemental sulfur was successfully adsorbed during the process and retained on the surface of the activated biochars. The biochars activated with 0.1 ml water dose presented a greater mass increase in the range of 0.04 to 0.05 g and an adsorption capacity in the range of 45 to 64 mg/g compared to the biochar samples activated with a water dose of 0.5 ml, for which the mass increase ranged from 0.034 to 0.04 g and the adsorption capacities ranged from 36 to 44 mg/g. This can imply that steam activation with greater activation water quantity may lead to excessive gasification, in which micropores are converted into mesopores or macropores. As a result, the specific surface area to retain more elemental sulfur can be reduced, thereby decreasing the area available for adsorption. 

\vspace{0.3em}

Among the biochars activated with 0.1 ml water dose, the highest mass increase of approximately 0.053 g and the highest adsorption capacity of approximately 64.3 mg/g were observed in the biochar pyrolyzed at 500 °C, followed by the sample pyrolyzed at 400 °C and 600 °C. This may possibly be attributed to further possible carbonization at 600 °C, which can lead to a reduction in oxygen-containing functional groups, although higher pyrolysis temperatures are generally considered advantageous for higher adsorption capacity, as they promote pore formation and result in a larger specific surface area. 
...
\newpage
In contrast, among the biochars activated with a 0.5 ml water dose, the biochar pyrolyzed at 600 °C showed the highest increase of approximately 0.04 g and an adsorption capacity of 43.884 mg/g, while the lowest values were observed for the sample produced by pyrolysis at 500 °C. This implies that the effect of the pyrolysis temperature on the adsorption capacity is also governed by the amount of water used during activation.
...
\begin{table}[H]
\centering
\caption{Adsroption capacity of the  CO$_2$-activated Algerian biochars.}
...
It was observed that the adsorption capacity of Algerian biochars activated with CO$_2$ decreased as the pyrolysis temperature decreased, with the highest adsorption capacity of approximately 24 mg/g observed for biochar produced by pyrolysis at 400 °C. This implies that CO$_2$ activation is favored in biochar pyrolyzed at lower temperatures.

\vspace{0.3em}

Compared to the adsorption capacity of CO$_2$-activated biochars from previous work, the adsorption capacities of biochars in this work are 2-3 times higher. Although the CO$_2$ activation is theoretically associated with the development of narrow micropores, Steam activation not only promotes the formation of micropores but also allows for the formation of mesopores and macropores, which facilitate the transport of elemental sulfur to the active adsorption sites. The fewer presence of oxygen-containing functional groups such as phenolic and carbonyl groups during CO$_2$-activation may also contribute to the lower adsorption capacity of the biochar, since they provide reactive sites for the adsorption \cite{Sajjadi2019}. This suggests that the adsorption capacity may be influenced not only by the surface area but also by the pore size distribution and the oxygen-containing functional groups.
...
\setresponsible{Regen Wijaya}
The BET analysis of pristine, as well as sulfur-loaded and activated French biochar samples, resulted in three parameters: the SSA, total pore volume, and mean pore diameter.  
...
Based on the results of the characterization of the physicochemical properties of biochar conducted in this study, the potential applications of the biochar produced are limited to use in adsorption, carbon storage, and soil amendment. The BET results show that the activated, sulfur-loaded biochar produced at a pyrolysis temperature of 500°C is the most promising biochar for adsorption-related applications, including the desulfurization of biogas, since it has the highest value of SSA and total pore volume among all biochars produced in this work. However, the activated, sulfur-loaded biochar produced at a pyrolysis temperature of 400°C might be promising as well, as discussed. 
...
With all of the results, analyzes, and discussions, this section weighs whether the thesis examined here; turning slaughterhouse digestate into a steam-activated; is worth pursuing beyond this work. The discussion here will remain qualitative, which means it rests on the material properties measured rather than on cost figures. For that, it would then need pilot-scale data that the present experiments cannot provide.
\vspace{0.3em}
