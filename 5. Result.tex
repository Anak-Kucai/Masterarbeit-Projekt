...
\footnotesize
\textit{Note.} ``$T_\mathrm{pyr}$'' refers to the pyrolysis temperature at which the biochars were produced. 
\end{minipage}
\end{table}
...
As shown in Figure~\ref{fig:pyrolysis-yield}, the biochar yields from both digestates behave identically,  with the replicate standard deviation at most 0.6 percentage points and below 0.1 for the entire French series (individual crucible masses in Table~\ref{tab:pyrolysis-yield-full}). Across the temperature range, the French digestate exhibited a higher solid yield, falling from 45.7\,\% at 400\,$^{\circ}$C to 42.2\,\% at 600\,$^{\circ}$C, whereas the Algerian digestate yielded between 37.4\,\% and 33.5\,\% over the same interval, as shown in both Table~\ref{tab:yield_summary} and Figure~\ref{fig:pyrolysis-yield}. The French biochar therefore exceeded the Algerian biochar by roughly eight to nine percentage points at every temperature. This difference may be attributed to  different feedstock compositions between both digestates rather than the pyrolysis process because both digestates were derived from slaughterhouse waste and pyrolyzed under identical conditions. The higher French biochar yield indicates a larger fraction of material retained in the solid phase, which points to differences in the organic and mineral balance of the two digestates arising from their respective origins and upstream digestion. This interpretation was addressed further in Section~\ref{sec:characterisation} based on the elemental and ash data, where the mineral content recovered from the coal is directly quantified.
...
The Algerian digestate runs also connect this work to the preceding project work mentioned before, in which the
same slaughterhouse waste derived digestate was pyrolyzed under comparable conditions. The present yields of 37.4, 34.4 and 33.5\,\% reproduce the earlier values of approximately 37, 34, and 31\,\% (Table~\ref{tab:projektarbeit-yield}) \parencite{MartinezHuete2024Projektarbeit}, confirming that the pyrolysis procedure is reproducible across the two experimental campaigns. Set in the wider context of that
study, the digestate from slaughterhouse waste falls in the same range as the grass-silage
...
\begin{table}[H]
\centering
\caption{Adsorption capacity of the  CO$_2$-activated Algerian biochars.}
...
\end{axis}
\end{tikzpicture}
\caption{Atomic H/C ratio of the pristine Biochars and the sulfur-loaded activated chars.}
...
\end{axis}
\end{tikzpicture}
\caption{Atomic N/C ratio of the pristine Biochars and the sulfur-loaded activated chars.}
\label{fig:chns-nc}
\end{figure}
...
Figure~\ref{fig: total pore volume} shows the total pore volume of both the pristine and the activated, sulfur-loaded French biochars, as measured by BET analysis. The pattern is identical to that of the SSA. The pore volume of the pristine samples also increased with rising pyrolysis temperature, with the highest total pore volume observed at 600 °C at approximately 0.028 cm$^3$\,g$^{-1}$, followed by 0.005 cm$^3$\,g$^{-1}$ at 500 °C and 0.003 cm$^3$\,g$^{-1}$ at 400 °C. As the temperature rose, further pore networks formed as a result of devolatilization.
\vspace{0.3em}

After activation and sulfur loading, the pore volumes increased significantly to 0.071~cm$^3$\,g$^{-1}$ at 400°C, 0.095~cm$^3$\,g$^{-1}$ at 500°C and 0.068~cm$^3$\,g$^{-1}$ at 60°C. The biochar pyrolyzed at 500°C remains favorable to obtain higher pore volume through steam activation, while The biochar pyrolyzed at 600°C exhibited the lowest total pore volume.
...
With all of the results, analyzes, and discussions, this section weighs whether the thesis examined here; turning slaughterhouse digestate into a steam-activated biochar; is worth pursuing beyond this work. The discussion here will remain qualitative, which means it rests on the material properties measured rather than on cost figures. For that, it would then need pilot-scale data that the present experiments cannot provide.
\vspace{0.3em}
