...
%Scanning electron microscopy (SEM) images a sample by rastering a focused electron beam across its surface and detecting the secondary and backscattered electrons generated by the beam–specimen interaction, producing a high-resolution, high-depth-of-field image of surface topography (Goldstein et al., 2018). For carbonaceous adsorbents, SEM is routinely used to visualize how pyrolysis and subsequent activation reshape the particle surface and macropore architecture: raw biochar typically shows a comparatively smooth, intact cellular structure inherited from the plant tissue, whereas increasing pyrolysis severity and, in particular, physical (steam) activation progressively expose and widen the vascular and cellular channels, producing a more open, porous, and irregular surface texture (Bardestani & Kaliaguine, 2018). SEM is therefore commonly reported alongside BET gas adsorption data, since the two techniques probe complementary aspects of the pore network: BET quantifies the total accessible surface area and micro-/mesoporosity, while SEM provides direct, qualitative visual evidence of macropore development, particle morphology, and — in the case of sulfur-loaded samples — the presence of sulfur deposits on or within the pore structure (Bardestani & Kaliaguine, 2018).
%%Where an EDX (energy-dispersive X-ray) detector is coupled to the SEM, elemental mapping can additionally be used to visualize the spatial distribution of the loaded sulfur across the carbon surface, which is useful for distinguishing between uniform monolayer-type deposition and localized pore-filling by elemental sulfur crystallites (Goldstein et al., 2018). 

%ini textnya dari chatgpt

\label{Characterization Methods of Biochar}
\subsubsection*{The Brunauer-Emmett-Teller (BET) analysis}

BET analysis is one of the analytical methods used to determine the specific surface area and porosity of a solid porous material. It is originally derived from the Langmuir model, which is also an analytical method to describe monomolecular layer adsorption, associated with chemisorption \cite{UniBayreuth2008BET}. BET analysis is used exclusively in the context of physical adsorption, since the amount of physically adsorbed gas indicates the specific surface area of the material and describes the formation of multiple molecular layers on the accessible surface of the material \cite{UniBayreuth2008BET}. In physical adsorption, the intermolecular interactions (van der Waals forces) between the adsorbent and the adsorbate are relatively weak, which allows for the formation of multiple molecular layers as well as complete reversible desorption \cite{UniBayreuth2008BET}. 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Abbildungen/Adsorptionisotherme.jpg}
...
The Equation to calculate the specific surface area of the biochar \cite{UniBayreuth2008BET}: 
