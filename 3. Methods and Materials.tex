...
For this thesis, the biochar carbon content is assumed to be approximately 60\% based on preceding project work. Based on this number, a stoichiometric calculation for the respective reaction indicates that, at a uniform tube temperature of 700 °C, only about 0.09 g of water ($\approx$ 0.09 mL at STP) would theoretically be required to gasify 0.06 g of carbon. In practice, however, the temperature is not uniform across the entire tube volume; the central hot zone reaches the set point while the tube extremities remain considerably cooler, so that a non-negligible fraction of the injected water condenses on the cooler tube walls and is not available for reaction with the biochar. The maximum temperature was also set to 700 °C, since the pipe joint could not withstand temperatures exceeding 700 °C.

\vspace{0.3em}

To test whether the 10\,\% mass-loss target could be met under these non-ideal conditions, a comparative trial was carried out with two water doses applied to 1\,g biochar samples from each pyrolysis temperature: 0.1\,mL (close to the theoretical minimum) and 0.5\,mL (a fivefold excess intended to compensate for steam losses to condensation). On the wet basis both doses sat near or below 10\,\%, and the losses were essentially identical between doses for the 500 and 600\,$^{\circ}$C biochars (see table~\ref{tab:activation-full}). After the moisture correction, however, the losses ran 10--22\,\%, so the 10\,\% bound was exceeded at every pyrolysis temperature. Section~\ref{subsec:results-activation} attributes the excess to residual devolatilization of chars produced below the 700\,$^{\circ}$C activation temperature. Since the bound was a precautionary target rather than a specification, exceeding it does not invalidate the runs; it means the mass loss cannot be read as gasified carbon alone.

\vspace{0.3em}

On this basis, the 0.5 mL of water per 1 g of biochar dose was adopted as the standard activation condition. All activated biochars used thereafter for the elemental and surface analyses described in \S~\ref{sec: Analyticalmethods} were produced under this condition. The 0.1 mL dose was retained only as a comparative reference for the activation study. The same dose ratio was additionally checked at twice the scale. In runs 10--12 (Table~\ref{tab:activation-full}), 2\,g of biochar was activated with 1.0\,mL of water, and the
