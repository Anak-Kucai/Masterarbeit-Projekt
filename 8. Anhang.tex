...
\begin{tabular}{ccccc}
\toprule
\thd{$T$ ($^{\circ}$C)} & \thd{Crucible} & \thd{Charge (g)} & \thd{Biochar (g)} & \thd{Yield (\%)} \\
\midrule
...
\toprule
\thd{Run} & \thd{$T_\mathrm{pyr}$ (°C)} & \thd{Sample (g)} & \thd{H$_2$O (mL)}
    & \thd{$M_0$ (g)} & \thd{$M_1$ (g)} & \thd{Loss, wet (\%)}
    & \thd{Loss, corr.\ (\%)} \\
\midrule
...
\footnotesize
\textit{Note.} Runs~1--3 were discarded
because of a reactor seal failure and are not reported.
\end{minipage}
\end{table}
...
\toprule
\thd{Material} & \thd{Rep.} & \thd{$m_0$ (g)} & \thd{$m_{\mathrm{ash}}$ (g)}
& \thd{HHV (MJ\,kg$^{-1}$)} & \thd{Ash (\%)} \\
\midrule
...
\toprule
\thd{Sample} & \thd{$T$ ($^\circ$C)} & \thd{C (\%)} & \thd{H (\%)}
& \thd{N (\%)} & \thd{S (\%)} & \thd{O (\%)} \\
\midrule
\multirow{3}{*}{Biochar}
...
\begin{minipage}{0.95\textwidth}
\footnotesize
\textit{Note.} Each element of Table~\ref{tab:chns-full} divided by the combustible fraction $(1-\text{ash})$, so that the values describe the organic matter alone. The sulfur-loaded activated chars are not listed, because their ash content was not determined and no ash-free basis can be calculated for them.
\end{minipage}
\end{table}

\begin{table}[ht]
\centering
\caption{Atomic ratios derived from the CHNS data. These ratios are independent of ash content and can therefore be given for all six samples.}
\label{tab:chns-ratios}
\renewcommand{\arraystretch}{1.15}
\setlength{\tabcolsep}{9pt}
\begin{tabular}{llcccc}
\toprule
\thd{Sample} & \thd{$T$ ($^\circ$C)} & \thd{H/C} & \thd{O/C}
& \thd{N/C} & \thd{S/C} \\
\midrule
\multirow{3}{*}{Biochar}
...
\begin{minipage}{0.95\textwidth}
\footnotesize
\textit{Note.} Ratios are calculated on an atomic basis from the contents of Table~\ref{tab:chns-full}. O/C cannot be given for the sulfur-loaded samples: oxygen was obtained by difference and therefore requires an ash content, which was not determined for those chars.
\end{minipage}
\end{table}
